...
Codex CLI is an offline author. It reads a tracked project-owned primer and writes the exact
declarative Expedition document consumed by learner-runtime. The repository contains no Codex/model
client, model port, prompt, compiler, package, installer, publication transaction, content database,
or build/runtime model call.
...
representation. A semantic document or referenced source-byte change changes the revision, while JSON
formatting and property order do not.
