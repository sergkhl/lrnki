...
- **Objective:** Resolve the two open caveats from the MiMo cutover (plan 2026-07-10-002,
  folded into `TODO.md` COMPLETED) with measurements, not guesses: decide whether MiMo's
  lower discovery recall loses real learning structure across mixed domains, and lock the
  known-fatal MiMo tool-schema shape behind a mechanical test. Tuning happens only if the
...
- **Execution profile:** Measure-first. U1 and U3 always execute; U2 executes only when U1's
...
- **Tail ownership:** The executor owns the audit run over all fixtures, human inspection of
  reported misses, the tune-or-accept decision, ADR-0018 amendment if a tune is adopted,
  durable memory/TODO updates, and final `docs/plans/` cleanup.
...
(trailing nullable property) from being reintroduced. The cache-pin question is already
answered from spend-log evidence and is recorded here, not implemented.

### Problem Frame

The 2026-07-10 cutover (evidence `tmp/2026-07-10-extraction-model-switch-mimo/evidence.md`)
accepted two caveats:
...
conditional sampling tune, the descriptor-shape test, ADR-0018 amendment if a tune is
adopted, memory/TODO/README repairs.
...
### Recorded evidence — cache pin payoff (measured 2026-07-10, no implementation unit)

Queried live during planning against `LiteLLM_SpendLogs` (dedicated `litellm` DB), MiMo calls
since the cutover:
...
- Per-row spend reconciliation: recorded `spend` matches the cache-discounted computation
  ...
  full-price `0.00267960`), so LiteLLM applies `cache_read_input_token_cost` correctly and
  Cost & timings reporting is accurate. A warm admission call costs ~63% less than unpinned
  full price.
...
The executor re-runs both queries during the gate and stores them with results in the
evidence folder.
