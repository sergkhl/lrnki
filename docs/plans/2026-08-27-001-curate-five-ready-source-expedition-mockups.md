...
### U9 — iOS Simulator, repository gate, and closure — complete
...
### U9 — Exact-revision iOS Simulator and closure gate — 2026-08-29

- Commit `9ac47eb` makes Guardian arrival wait until Activity or Support Path returns control to the trail; the focused regression proves no arrival over an active sheet and one arrival after close. Xcode 26.6 then built and installed its fresh 4,171-module Debug bundle with zero errors on an iPhone 17 Pro / iOS 26.5 Simulator against the real supervisor-free local API and development Postgres.
- Inspected Simulator screenshots covered the exact five-title catalog and centralized source/license dialog, then the first source-grounded lesson and sealed correct activity for every path. Critical Thinking completed all 31 reads and 31 correct responses through the continuous acquisition route; the repaired one-crystal Guardian arrival rendered only after Activity closed, and app restarts preserved progress. The positive database control was one user, five expeditions, 35 lesson reads, 35 responses, and zero awards; exact teardown returned all auth and learner projections to zero.
- The final model-free projection retained five ordered catalog entries, five source/run/graph/enrichment closures, 31/44/37/31/40 qualified stops, 28/42/29/26/16 package Concept projections, and one 141-Concept registry with zero auth, learner, award, or operation rows. `pnpm test:db` passed its migration/reset matrix and DB workspace suites; `pnpm check` passed schema parity, every typecheck/test, lint, both production web builds, 70/70 intercepted scenarios, coordination checks, and whitespace. Screenshots remained inspection-only under gitignored `tmp/`. This is fresh Debug iOS Simulator, local development database, real local API, local build, and intercepted-web evidence only — not Android, automated native, deployed, distributable, physical-device, production, or release authority. The primers remain model-authored project playtest sources accepted from general model knowledge without independent claim verification; this result does not generalize to arbitrary-source ingestion.
