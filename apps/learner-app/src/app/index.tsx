...
// menu trigger (Duel / Board / logout as sheet rows), the partitioned expedition
// journal, the Duel goal card, and the one-splash-per-visit coordinator.
export default function JournalPage() {
  ...
  const duelSetup = useQuery({ ...duelSetupQuery, enabled: signedIn });
  ...
  return (
    <Screen>
      ...
      <JournalSplashCoordinator
        ...
        duelUnlocked={duelSetup.isPending ? undefined : duelSetup.data?.unlocked ?? null}
        onEnterDuel={() => router.push("/duel")}
      />
    </Screen>
  );
