...
function graded(conceptId: string, outcome: JudgedOutcome, source: "synthetic" | "human" = "synthetic"): ResponseLogRow {
  return { responseId: `r${++seq}`, learnerStateRef: "L1", cardId: `card-${conceptId}`, conceptId, signalType: "graded", selfReportRating: null, judgedOutcome: outcome, gradedScore: outcome === "correct" ? 1 : outcome === "partial" ? 0.5 : 0, evidenceWeight: 1, responseSource: source, graderIdentity: "kg-independent-judge", attemptSeq: seq, batchId: null, submittedAnswer: "answer", createdAt: new Date().toISOString() };
}

test("detectConflicts flags a concept claimed known but graded incorrect (Covers R16)", () => {
  const conflicts = detectConflicts([selfReport("cA", "good"), graded("cA", "incorrect")]);
  assert.equal(conflicts.length, 1);
  assert.equal(conflicts[0].kind, "claimed_known_but_failed");
  assert.equal(conflicts[0].conceptId, "cA");
});

test("detectConflicts does not flag a concept where self-report and graded agree", () => {
  assert.equal(detectConflicts([selfReport("cA", "good"), graded("cA", "correct")]).length, 0);
});

test("detectConflicts flags the reverse: claimed unknown but graded correct", () => {
  const conflicts = detectConflicts([selfReport("cB", "again"), graded("cB", "correct")]);
  assert.equal(conflicts.length, 1);
  assert.equal(conflicts[0].kind, "claimed_unknown_but_passed");
});

test("detectConflicts ignores concepts with only one signal type", () => {
  assert.equal(detectConflicts([selfReport("cA", "good"), graded("cB", "incorrect")]).length, 0);
});
...
function fakeResponseLog(initial: ResponseLogRow[]): { store: ResponseLogStorePort; rows: ResponseLogRow[] } {
  ...
  const store: ResponseLogStorePort = {
    ...
    async listForLearnerConcept(ref, c) { return rows.filter((r) => r.learnerStateRef === ref && r.conceptId === c); },
...
test("resubmit appends a new graded row, leaves the original synthetic row intact, and recomputes the path (Covers AE5, R15)", async () => {
  const original = graded("cA", "incorrect", "synthetic"); // an earlier synthetic graded row
  ...
  const result = await resubmitAndRecompute({
    learnerStateRef: "L1",
    card: { cardId: "card-cA", conceptId: "cA", question: "Q?", answerKey: "A" },
    declaredDomain: "d",
  ...
  // a new human graded row was appended for cA.
  const humanRows = log.rows.filter((r) => r.responseSource === "human" && r.conceptId === "cA");
