// forked from design-sync lib/dts.mjs — one line: `strictNullChecks: true` in projectFor's
// compilerOptions (marked "fork" below). Everything else is the bundled module verbatim.
//
// Upstream builds its ts-morph project with `strict: false`, which turns strictNullChecks
// OFF, so the checker absorbs `null` into every union. This DS declares ~30 semantically
// load-bearing nullable props — `Progress.fraction: number | null` means INDETERMINATE,
// `SupportPathsPanel.busyTerm: string | null` means "nothing in flight",
// `CrystalFormationScene.focus: VistaFocus | null` means "nothing focused" — and all of
// them emitted as plain non-null types, silently telling the design agent that a real and
// meaningful state does not exist. `cfg.dtsPropsFor` would fix only the components someone
// remembered to enumerate, as hand-written bodies that drift from source.
//
// Verified: turning strictNullChecks on recovers the unions and adds NO `| undefined` noise
// to optional props (spot-checked Button, Progress, SupportPathsPanel before/after).
...
function projectFor(pkgDir, typesRoot) {
  ...
  const project = new Project({
    ...
    compilerOptions: {
      ...
      skipLibCheck: true,
      strict: false,
      // fork: see header — recovers semantically load-bearing `| null` unions.
