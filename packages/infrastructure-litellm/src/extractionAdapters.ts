...
export class LiteLlmEvidenceProfileExtractionAdapter implements ConceptConditionedEvidenceProfileExtractionPort {
  ...
  async extract(input: {
    ...
    const system = [
      ...
      "DEFINITION PASSAGES: one or more verbatim passages that establish what the subject concept MEANS. A definition passage need NOT use a literal 'X is Y' form — apposition ('—a discrepancy known as the generalization gap'), a 'means'/'refers to'/'known as' construction, or any meaning-bearing sentence qualifies, as long as the passage conveys the concept's meaning. The following NAME or INVOKE the concept but do NOT define it, so none is a definition passage: a bare repetition of its name, a section heading or title, a citation or bibliographic reference, or a passage that only mentions, uses, applies, or describes the concept's effects or behaviour in passing without stating what it IS. When more than one candidate block refers to the concept, PREFER the block that states its defining properties, distinguishing criteria, or mechanism over one that merely names, applies, or describes it — do not default to the first or most prominent mention; quote the sentence that actually explains the concept. Include at least one; a concept the source never gives meaning to does not belong here.",
