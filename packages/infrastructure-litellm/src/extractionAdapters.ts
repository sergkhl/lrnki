...
export class LiteLlmConceptAdmissionAdapter implements ConceptAdmissionPort {
  ...
  async admit(input: { document: StructuredDocument; declaredDomain: string; candidates: DiscoveredCandidate[] }): Promise<AdmissionProposal[]> {
    const system = [
      ...
      "Source-role test: ask 'if I were building a concept graph FOR the Declared Domain, would a learner of THAT domain study this concept?' If the concept is really taught BY another field and merely appears here as illustration, it is out_of_domain_illustration. Example: in an EDUCATIONAL-TECHNOLOGY paper that builds knowledge graphs over computer-science and database course materials, the CS/DB concepts it ingests as sample content — merge sort, recursion, divide-and-conquer, dynamic programming, greedy algorithms, SQL clauses (WHERE/ORDER BY/FROM), FOREIGN KEY, PRIMARY KEY, referential integrity — are out_of_domain_illustration, NOT educational-technology concepts. The ed-tech concepts are the framework, signals, roles, and graph-construction method themselves.",
      ...
      "Rust ownership calibration: 'Copy trait' is not merely an operation name when the source explains it as a trait/category that changes move behavior; admit it as core if the evidence supports the three tests. By contrast, Stack, Heap, heap allocation, String examples, Drop/drop, clone, and concrete Copy-implementing primitive types are usually supporting context or examples in an ownership lesson unless the source independently teaches them as principal learning objectives with two aspects.",
    ...
    const selectionUser = [
      ...
      "Illustrative-example demotion (apply narrowly): read each candidate's evidence quotes and the heading they come from. Demote a candidate as illustrative ONLY when EVERY substantive evidence quote for it is drawn from a case-study / example / evaluation / demo section AND the source never explains the concept in its own main exposition — i.e. it is merely a vehicle for demonstrating the source's contribution (e.g. a method paper that only lists 'Dynamic Programming' or 'Greedy Algorithms' as nodes in its §5 case-study graph and never teaches them). Do NOT demote a concept the source explains in its main text just because it ALSO appears in a case study, evaluation, or example. When in doubt and the source teaches the concept substantively, retain it.",
      ...
      "Sparse-source calibration: in a Rust ownership lesson, retain distinct mechanisms the source actually teaches as learner objectives — for example variable scope, ownership, move semantics, and the Copy trait can all be core when each has two substantive aspects. Do not demote Copy trait merely because it is a supporting mechanism of move semantics, and do not demote variable scope merely because it scaffolds ownership.",
      "Redundancy calibration for that same lesson: do not keep both a composite section label and its atomic children. Stack/heap, heap allocation, String type, drop/clone operations, and function-return section headings are usually supporting context unless the source's principal explanatory structure centers on them independently.",
