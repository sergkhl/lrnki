...
async function seed(sql: ReturnType<typeof createDatabaseClient>, learnerStateRef: string) {
  ...
  await sql`INSERT INTO rejected_study_items (rejected_study_item_id, graph_version_id, enrichment_id, derived_node_id, item_type, reason, config_hash) VALUES (${randomUUID()}, ${graphVersionId}, ${enrichmentId}, ${nodeB}, 'option_select', 'no usable grounding for an option-select item', 'cfg')`;
