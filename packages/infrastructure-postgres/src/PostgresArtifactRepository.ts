...
export class PostgresArtifactRepository implements ArtifactRepositoryPort {
  ...
  async append<TPayload>(artifact: ArtifactEnvelope<TPayload>): Promise<void> {
    await this.sql`
      ...
      VALUES (${artifact.artifactId}, ${artifact.artifactType}, ${artifact.schemaVersion}, ${artifact.runId ?? null}, ${artifact.graphVersionId ?? null}, ${artifact.producer}, ${artifact.producerVersion}, ${artifact.configHash}, ${this.sql.json(artifact.payload as Parameters<Sql["json"]>[0])})
