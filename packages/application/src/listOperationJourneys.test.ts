...
test("groups multi-source document journeys, shared enrichment study-items ids, and ungrouped leftovers", async () => {
  const result = await listOperationJourneys({
    ...ports({
      details: [
        ...
        detail({ operationId: "canonical-1", operationType: "canonicalization", startedAt: "2026-07-08T00:01:30.000Z", completedAt: "2026-07-08T00:01:50.000Z" }),
      ...
      lineages: [{ enrichmentId: "enr-1", graphVersionId: "graph-1", canonicalizationOperationId: "canonical-1", extractionRunIds: ["run-a", "run-b"] }],
  ...
  assert.deepEqual(
    ...
    ["extraction:run-a", "extraction:run-b", "canonicalization:canonical-1", "minting:graph-1", "enrichment:enr-1", "study_items:enr-1"]
  );
...
test("groups synthetic journeys without a graph version and derives running duration", async () => {
  const result = await listOperationJourneys({
    ...ports({
      ...
      lineages: [{ enrichmentId: "enr-topic", graphVersionId: null, canonicalizationOperationId: null, extractionRunIds: [] }],
...
test("failed member makes the whole journey failed", async () => {
  const result = await listOperationJourneys({
    ...ports({
      ...
      lineages: [{ enrichmentId: "enr-1", graphVersionId: "graph-1", canonicalizationOperationId: null, extractionRunIds: ["run-a"] }]
    }),
    now: new Date("2026-07-08T00:01:00.000Z")
  });
