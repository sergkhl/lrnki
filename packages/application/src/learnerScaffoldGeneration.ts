...
import type { ScaffoldContentDraft, ScaffoldContentPort, ScaffoldDetourStorePort, ScaffoldOutlinePort } from "@lrnki/ports";
...
export async function runScaffoldGeneration(
  ...
  for (const proposed of outline.steps.slice(0, 3)) {
    ...
    try {
      draft = await deps.content.generate({ declaredDomain: context.declaredDomain, label: proposed.label, groundingText: grounded.groundingText });
    } catch {
      continue;
    }
    const payload = buildScaffoldNodePayload(proposed.label, draft, newId);
    if (!payload) continue;
    steps.push({ scaffoldStepId: newId(), ordinal: steps.length, kind: "generated", payload, lessonReadAt: null });
